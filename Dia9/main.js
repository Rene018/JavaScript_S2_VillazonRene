...
const nameValue = nameb.value;
const pokeImg = document.getElementById("img-cont");
const btnNext = document.getElementById("next");
const btnPrev = document.getElementById("prev");
const pokeId = document.getElementById("pokeid");
let cont = 0;
nameb.addEventListener("keypress", function (event) {
  if (event.key === "Enter") {
    event.preventDefault();
    console.log("holis");
    cont = parseInt(nameb.value);
    getSingleCharacter(nameb.value);
  }
});
function getSingleCharacter(name) {
  let getCharacterById = new XMLHttpRequest();

  getCharacterById.open(
    "GET",
...
function getCharacterByBtn(name) {
  let getCharacterById = new XMLHttpRequest();
